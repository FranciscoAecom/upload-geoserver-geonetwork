param(
  [string]$OutputFolder = "C:\Users\RibeiroF\Downloads\figuras_rl_car_modelo",
  [int]$Width = 140,
  [int]$Height = 92,
  [int]$Scale = 4,
  [int]$DilatePixels = 1,
...
$tmp = Join-Path $env:TEMP ("rl_car_figures_" + [guid]::NewGuid().ToString("N"))
...
function Set-GreenFill {
  ...
  $green = [System.Drawing.Color]::FromArgb(255, 8, 125, 3)
...
try {
  ...
  foreach ($uf in $targetStates) {
    ...
    $gpkg = Join-Path $base ("rl_car_{0}\SICAR\20260301\00\pol_pcd_rl_car_{0}_20260301.gpkg" -f $uf)
    ...
    $tif = Join-Path $tmp ("rl_car_$uf.tif")
    $png = Join-Path $OutputFolder ("rl_car_$uf.png")
    ...
    & $gdalRasterize -q -of GTiff -ot Byte -ts $hiWidth $hiHeight `
      ...
      -init 255 -burn 8 -burn 125 -burn 3 `
      -l $layer $gpkg $tif
    ...
    Set-GreenFill -ImagePath $png -DilatePixels $DilatePixels
  }
} finally {
